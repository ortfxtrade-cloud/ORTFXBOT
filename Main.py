...
def quick_scan_single(symbol):
    try:
        ...
        diff_1m_prev = m_1m.iloc[-2] - s_1m.iloc[-2]

        # Detect latest 1m cross direction
        is_1m_bull_cross = (diff_1m_prev <= 0) and (diff_1m_now > 0)
        is_1m_bear_cross = (diff_1m_prev >= 0) and (diff_1m_now < 0)

        settings = get_effective_settings(symbol)
        rsi_val = rsi.iloc[-1]

        is_bull = (prev_diff_before < 0) and (prev_diff > 0) and is_1m_bull_cross
        is_bear = (prev_diff_before > 0) and (prev_diff < 0) and is_1m_bear_cross
        ...
        if is_bull and settings["rsi_buy_min"] <= rsi_val <= settings["rsi_buy_max"]:
            return "BUY", {"symbol": symbol, "macd_5m": m.iloc[-1], "signal_5m": s.iloc[-1],
                           ...
                           "diff_1m": diff_1m_now, "rsi_5m": rsi_val}
...
# --- Scanner Engine ---
def scanner_engine():
    global alert_cooldowns, signal_log, full_signal_messages, martingale_jobs, compression_counter, compression_blocked, spread_blocked_1m, spread_blocked_5m
    while True:
        if STATE["running"]:
            with data_lock:
            ...
            for symbol in current_pairs:
                try:
                    ...
                    confirm_bull = (prev_diff_before < 0) and (prev_diff > 0) and (abs(prev_diff) >= 0.00001) and is_1m_bull_cross and (settings["rsi_buy_min"] <= rsi_val <= settings["rsi_buy_max"])
                    confirm_bear = (prev_diff_before > 0) and (prev_diff < 0) and (abs(prev_diff) >= 0.00001) and is_1m_bear_cross and (settings["rsi_sell_min"] <= rsi_val <= settings["rsi_sell_max"])
...
def record_feedback(msg_id, result, delay_sec=0, reason="", notes="", analysis=""):
    ...
    for entry in signal_log:
        if entry.get("msg_id") == msg_id:
            ...
            # Track loss streak (for statistics only; no automatic threshold adjustment now)
            if result == "LOSS":
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port)￼Enter        else:
                kb = get_pairs_keyboard(pairs, "remove", page)
                bot.edit_message_text("❌ *Select pair to remove:*", call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
