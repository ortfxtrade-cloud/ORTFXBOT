...
def calculate_strategy(df):
    ...
    # Corrected RSI with Wilder's smoothing
    delta = df['Close'].diff()
        ...
        if STATE["running"]:
            ...
            for symbol in current_pairs:
                try:
                    ...
                    # 5-minute cross + gap + 1m confirmation (NO PRE-ALERT)
        ...
        if data == "main_menu":
          ...
          pairs = list(STRATEGY_PAIRS)
            ...
            with data_lock:
                pairs = list(STRATEGY_PAIRS)
            if not pairs:
                bot.edit_message_text("📭 No pairs to remove.", call.message.chat.id, call.message.message_id, reply_marimport os
import re
import time
import random
import threading
    ...
    try:
        if data == "main_menu":
            ...
            help_text = (
                "🤖 *Forex Scanner Bot*\n\n"
                "*Signals:*\n"
                "🟢/🔴 MACD Cross + 1m Gap Confirmed\n\n"
                "*Filters:*\n"
                "🚫 Spread: 5m check, 1 hour silent block\n"
                "📏 1m Gap: Dynamic per pair (15% of avg)\n\n"
                "*Strategy:* MACD Crossover + RSI Filter\n"
                "*BUY:* Diff - → + | Gap | 1m gap | RSI 30-45\n"
                "*SELL:* Diff + → - | Gap | 1m gap | RSI 55-70\n\n"
                "*Commands:*\n"
                "/start - Launch bot\n"
                "/menu - Main menu\n"
                "/add SYMBOL - Add pair\n"
...
def start(m):
    if str(m.chat.id) == CHAT_ID:
        bot.send_message(
            ...
            "🟢/🔴 MACD Cross + 1m Gap Confirmed\n"
            "🚫 Spread Filter: Silent (use Blocked button)\n\n"
            "Select an option:",
            reply_markup=get_main_menu(),
            parse_mode="Markdown"
        )
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port)￼Enterkup=get_main_menu())
            else:
                kb = get_pairs_keyboard(pairs, "remove", page)
                bot.edit_message_text("❌ *Select pair to remove:*", call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
