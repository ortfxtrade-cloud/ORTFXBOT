...
def quick_scan_single(symbol):
    try:
        ...
        # Updated RSI thresholds: BUY 30-40, SELL 50-70
        if is_bull and 30 <= rsi.iloc[-1] <= 40: return "BUY", result
        elif is_bear and 50 <= rsi.iloc[-1] <= 70: return "SELL", result
        ...
        if STATE["running"]:
            ...
            for symbol in current_pairs:
                try:
                    ...
                    # Updated RSI thresholds: BUY 30-40, SELL 50-70
                    confirm_bull = (prev_diff_before < 0) and (prev_diff > 0) and (abs(prev_diff) >= 0.00001) and is_1m_bullish and (30 <= rsi.iloc[-1] <= 40)
                    confirm_bear = (prev_diff_before > 0) and (prev_diff < 0) and (abs(prev_diff) >= 0.00001) and is_1m_bearish and (50 <= rsi.iloc[-1] <= 70)
...
# --- Gemini API helper (renamed to ask_ai_core) ---
...
# Chat/Debug mode catch-all (uses ask_ai_core)
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port)￼Enter                            new_row.append(InlineKeyboardButton("🔍 More Details", callback_data="showdetails_"))
                        else:
                            new_row.append(btn)
                    if new_row:
                        new_kb.add(*new_row)
