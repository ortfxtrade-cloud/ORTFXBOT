                    ...
                    if confirm_bull or confirm_bear:
                        if time.time() - alert_cooldowns.get(symbol, 0) > 300:
                            ...
                            kb.add(
                                ...
                                InlineKeyboardButton("❌ LOSS", callback_data=f"loss_"),
...
# --- Next-step handler for Feedback button ---
def process_feedback_reply(message, msg_id):
    ...
    if success:
        ...
        bot.reply_to(message, "❌ Could not find the signal.")
...
def handle_callback(call):
    ...
    try:
        if data.startswith("win_") or data.startswith("loss_"):
            ...
            success, extra_msg = record_feedback(msg_id, result)
            if success:
                ...
                try:
                    ...
                    for row in call.message.reply_markup.keyboard:
                        new_row = []
                        for btn in row:
                            if btn.callback_data.startswith("win_") or btn.callback_data.startswith("loss_"):
                                new_row.append(InlineKeyboardButton("✅ RECORDED" if result=="WIN" else "❌ RECORDED", callback_data="none"))
                            else:
                                new_row.append(btn)
                        if new_row:
                            new_kb.add(*new_row)
        ...
        # --- All other standard handlers (watchlist, add/remove, etc.) remain exactly as before, but ensure correct indentation ---
        elif data == "main_menu":
          ...
          bot.edit_message_text(msg, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
            ...
            page = int(data.split("_")[-1]) if data.startsimport os
import re
import time
import random
import threading
    ...
    try:
        ...
        # --- All other standard handlers (watchlist, add/remove, etc.) remain exactly as before, but ensure correct indentation ---
            ...
            help_text = (
                "🤖 *Forex Scanner Bot*\n\n"
                "*Signals:*\n"
                "Concise alert with expandable details.\n"
                "Tap 🔍 More Details for full analysis.\n\n"
                "*Feedback:*\n"
                "✅ WIN / ❌ LOSS – instant\n"
                "💬 Feedback – add delay & reason\n\n"
                "*Menu:*\n"
                "📈 Stats / ⏱️ Entry / 🚫 Blocked"
            )
            kb = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Main Menu", callback_data="main_menu"))
...
def start(m):
    if str(m.chat.id) == CHAT_ID:
        bot.send_message(m.chat.id, "🚀 *Forex Scanner Online*\n\n"
                         "Concise signals – tap 🔍 for details.\n"
                         "Use WIN/LOSS buttons to train me.\n\n"
                         "Select an option:", reply_markup=get_main_menu(), parse_mode="Markdown")
        ...
        if len(ticker.history(period="1d")) > 0:
            with data_lock:
                if s not in STRATEGY_PAIRS:
                    ...
                    added.append(s)
    bot.reply_to(m, f"✅ Added: {', '.join(added)}" if added else "❌ No valid symbols found.", reply_markup=get_main_menu())
    ...
    with data_lock:
        for s in syms:
            if s in STRATEGY_PAIRS:
                ...
                removed.append(s)
    bot.reply_to(m, f"🗑️ Removed: {', '.join(removed)}" if removed else "❌ No matching pairs found.", reply_markup=get_main_menu())
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port)￼Enterwith("page_info_") else 0
            with data_lock:
                pairs = list(STRATEGY_PAIRS)
            kb = get_pairs_keyboard(pairs, "info", page)
            bot.edit_message_text(f"📋 *Watchlist ({len(pairs)} pairs)*", call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
